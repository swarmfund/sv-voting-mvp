...
const testInstantiation = async function () {
  try {
    // const newProvider = TruffleProvider.create({provider: TestRPC.provider({port: 34839, accounts: S.map(_ => ({balance: "0xfffffffff999999f"}), S.range(0,200))})});
    // const web3 = new Web3(newProvider);
    // accounts = await (new Promise(res => web3.eth.getAccounts((e, s) => res(s))));
    
    // VotingCF.setProvider(newProvider);
    // VotingCF.defaults({from: accounts[0]});

    var startTime = Math.round(Date.now() / 1000) + 2;
    var endTime = startTime + 600;
    var shortEndTime = 0;

    let vc = await VotingCF.new(startTime, endTime, hexPk, true);
...
const testABallot = async function (_vc = S.Nothing, account = S.Nothing) {
...
contract('SwarmVotingMVP', function(_accounts) {
  ...
  it("should instantiate correctly", testInstantiation);
